...
public class AjoutProduit extends BaseForm {
    ...
    public AjoutProduit(com.codename1.ui.util.Resources resourceObjectInstance) {        
        ...
        TextField tfNom = new TextField("","Nom du produit");
        TextField tfdescProd = new TextField("","Description du produit");
        TextField tfquanProd = new TextField("","Quantité du produit");
        TextField tfprixProd = new TextField("","Prix du produit");     
        ComboBox<String> combo = new ComboBox<>();
        Button btnValider = new Button("Ajouter");
        btnValider.setUIID("InboxNumber3");
        btnValider.addActionListener(new ActionListener() {
            @Override
            //|| (combo.getSelectedItem().isEmpty())
            public void actionPerformed(ActionEvent evt) {
                if ((tfNom.getText().length()==0)||(tfprixProd.getText().length()==0))
                    Dialog.show("Alert", "Veuillez remplir tous les champs", new Command("OK"));
                else
                {        
                        Produit u;
                        u = new Produit(tfNom.getText(),tfdescProd.getText(),Float.parseFloat(tfquanProd.getText()), Float.parseFloat(tfprixProd.getText()));
                        System.out.println(u);
                        if( ServiceProduit.getInstance().addProduit(u))
                            Dialog.show("Succes", "Votre produit a été ajouté avec succès", new Command("OK"));
                     new ProduitViewBack(resourceObjectInstance).show();       
                    }   
                }
        });
        
        addAll(tfNom,tfdescProd,tfquanProd,tfprixProd,btnValider);    
...
protected String saveFileDevice(String file,String ext) throws URISyntaxException
{
URI uri;
uri = new URI(file);
